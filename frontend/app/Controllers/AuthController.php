...
class AuthController extends TamhorAuth
{
    public function register()
    {
        $rules = [
            ...
            'username' => [
                'rules' =>
                    'required|min_length[6]|max_length[200]|is_unique[users.username]',
                'errors' => [
                    'is_unique' =>
                        'This Username already use.',
                ],
            ],
            'password' =>
                'required|min_length[6]|max_length[200]',
            'confpassword' => 'matches[password]',
        ];

        if ($this->validate($rules)) {
            $data = [
                'fullname' => $this->request->getVar(
                    'fullname'
                ),
    ...
    {
        $data = $this->users
            ->where('activate_token', $activate_key)
            ->first();
        if ($data != null) {
            $update = [
                'is_active' => 1,
