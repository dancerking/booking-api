...
class Availability extends APIBaseController
{
	...
	public function index()
	{
        ...
        if(!$this->validate($rules)) return $this->notifyError('Input data format is incorrect', 'invalid_data', 'availability');
        ...
        return $this->respond([
            'availability_type' => $availability_type == null ? [] : $availability_type,
        ]);
	}
    ...
    public function update($id = null)
    {
        ...
        if(!ctype_digit((string)$type_availability_id)) {
            return $this->notifyError('Type availability id format is incorrect', 'invalid_data', 'availability');
        }
        if(!$this->validateDate($type_availability_day)) {
            return $this->notifyError('Date format is incorrect', 'invalid_data', 'availability');
        }
        if(!ctype_digit((string)$type_availability_qty)) {
            return $this->notifyError('Type availability qty format is incorrect', 'invalid_data', 'availability');
        }
        if(!ctype_digit((string)$type_availability_msa)) {
            return $this->notifyError('Type availability msa format is incorrect', 'invalid_data', 'availability');
        }
        if(!ctype_digit((string)$type_availability_coa)) {
            return $this->notifyError('Type availability coa format is incorrect', 'invalid_data', 'availability');
        }
        if(!ctype_digit((string)$type_availability_cod)) {
            return $this->notifyError('Type availability cod format is incorrect', 'invalid_data', 'availability');
        }
        ...
        if(!$type_availability_model->update($type_availability_id, $data)) {
           return $this->notifyError('Failed update', 'failed_update', 'availability');
        }
        return $this->respond([
