...
class PhotoContentModel extends Model
{
    ...
    public function get_level1_photo($host_id) {
        $query   = $this->db->query('SELECT photo_content_id, photo_content_url, photo_content_status
        FROM photo_contents
    ...
    public function get_level2_photo($host_id) {
        ...
        $query   = $this->db->query('SELECT photo_content_id, photo_content_connection, photo_content_url, photo_content_status
        FROM photo_contents
        WHERE photo_content_level = "2" AND photo_content_host_id = ' . $host_id);
        $results = $query->getResult();
        foreach($results as &$result) {
            $query = $this->db->query('SELECT type_mapping_name
            FROM types_mapping
    ...
    public function get_photo_contents($host_id, $record_status) {
        $query   = $this->db->query('SELECT photo_content_id, photo_content_level, photo_content_connection, photo_content_url
        FROM photo_contents
        WHERE photo_content_status = ' . $record_status . ' AND  photo_content_host_id = ' . $host_id);
        $results = $query->getResult();
        foreach($results as &$result) {
    ...
    public function is_existed_id($id) {
        $query = $this->db->query('SELECT photo_content_id FROM photo_contents WHERE photo_content_id = ' . $id);
        $results = $query->getResult();
        return $results;
    }
}
